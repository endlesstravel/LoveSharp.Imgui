...
namespace Love.Imgui
{
    public class Renderer
    {
        ...
        public Renderer(string fontPath, float fontSize, Func<string, Image> imageLoader = null)
        {
            // 初始化引擎
            EngineConfigure.Init();
        ...
        void UpdateInput(float dt)
        {
            ...
            // windows
            {
                io.DisplaySize = new Vector2(Graphics.GetWidth(), Graphics.GetHeight());
                io.DisplayFramebufferScale = new Vector2(1f, 1f);
            }

            // mouse
            {
                io.MousePos = new Vector2(Mouse.GetX(), Mouse.GetY());
                for (int i = 0; i < 5; i++)
